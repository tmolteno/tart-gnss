...
- **ACR C/N0 tables calibrated for one-bit data** — the lookup tables were
  generated by Monte Carlo on *unquantized* signals at 1 sample/chip, but
  every TART observation is one-bit quantized (16.368 MHz).  One-bit
  sampling halves the recovered coherent amplitude (factor sqrt(2/π) in
  the weak-signal limit), so the unquantized calibration biased C/N0
  estimates ~2-3 dB low — a true 40 dB-Hz signal estimated at ~37 dB-Hz,
  with most estimates pinned at the 33 dB-Hz table floor.  The generator
  (`examples/gen_acr_tables.rs`) now simulates the production path —
  16.368 MHz sampling through `simulator::quantize_bipolar_demean` — and
  the tables in `src/acr_tables.rs` were regenerated (strictly monotonic,
  4-decimal r_A).  The four `test_cn0_acr_end_to_end` tests now exercise
  the quantized path at the production sampling rate.
- **Per-constellation ACR C/N0 lookup tables** — the single lookup table was
  computed for T_coh = 1 ms, N_c = 1, GPS L1 C/A, but every acquisition mode
  integrates two code periods, and the ACR ratio depends on the coherent
  integration time through the peak's non-centrality λ0 = 2·Nc·T_coh·C/N0
  (Ma et al. 2024, eq. 29).  Applying the 1 ms table to 2/8/20 ms
  integrations overestimated C/N0 by ~3 dB (GPS/SBAS/QZSS), ~9 dB
  (Galileo) and ~13 dB (BeiDou/L1C).  Each constellation now uses a table
  generated by Monte Carlo simulation of the production acquisition
  functions at its actual T_coh and code family
  (`src/acr_tables.rs`, `examples/gen_acr_tables.rs`):
  - `GPS_L1_CA_ACR_TABLE` — GPS L1 C/A, SBAS, QZSS (T_coh = 2 ms)
  - `GALILEO_E1C_ACR_TABLE` — Galileo E1-C pilot (T_coh = 8 ms)
  - `BEIDOU_B1C_ACR_TABLE` — BeiDou B1C pilot (T_coh = 20 ms)
  - `GPS_L1C_ACR_TABLE` — GPS L1C pilot (T_coh = 20 ms)
  End-to-end tests inject a known C/N0 and require the median estimate
  within ±1 dB (measured: +0.1 to +0.8 dB).
- **Single-bin frequency search** — a search band narrow enough to produce
  one frequency bin divided by zero (`2·search_band/(n_freq_bins−1)` with
  `n_freq_bins == 1`), producing a NaN bin centre.  All six acquisition
  modules now handle the single-bin case (used by the C/N0 end-to-end
  tests).
- **Sign flips between the two integrated periods** — a pilot secondary
  code (Galileo E1-C 25-chip, GPS L1C overlay) or navigation data bit
  (C/A signals) can flip sign between the two integrated code periods;
  the flip lands at the code epoch, so a window containing one cancels
  the two-period correlation completely and the satellite was missed
  (≈50% of Galileo windows, ≈10% of GPS windows).  The acquisition is
  now dual-hypothesis: it correlates against both `[c, c]` and `[c, -c]`
  (second period negated) and keeps the stronger, so a flipped window
  still yields at least half the coherent peak.  The wiped-signal forward
  FFT is shared between the hypotheses; only the conjugate multiply and
  inverse FFT are repeated.  End-to-end tests inject a flip at the code
  epoch and require the median C/N0 estimate in the [-4.0, -0.5] dB band
  below the injected value (the flipped-window peak is inherently 0 to
  -6 dB low, median -2.5 dB, without secondary-code synchronisation).

### Changed

- **Crate restructured as lib + bin** — the acquisition modules moved to
  `src/lib.rs` (`tart_gnss_acquire` library crate) so the ACR table
  generator example and tests can use the production acquisition
  functions; `src/main.rs` is now a thin CLI over the library.
  `CombinedOutput` moved into the library; `beidou::b1c_code_resampled`
  and `l1c::l1c_code_resampled` are re-exported.
- **`acr::estimate_cn0` takes the table as an argument** —
  `estimate_cn0(r_a, table)`; callers pass the table for their
  constellation.
- **ACR tables regenerated with the dual-hypothesis acquisition** — the
  tables in `acr_tables.rs` were regenerated by
  `examples/gen_acr_tables.rs` so the r_A → C/N0 mapping matches the
  (slightly changed) peak statistics of the two-hypothesis search; the
  generator now enforces strictly monotonic tables.
