...
mod tests {
    ...
    fn test_cn0_acr_end_to_end() {
        ...
        // exercised: no flip (primary `[c, c]` wins, ±1 dB) and an
        // overlay-code flip at the code epoch (`[c, -c]` wins).  A flipped
        ...
        let period = (fs * L1C_CODE_PERIOD) as usize; // 163680 samples per 10 ms
        let f0 = 100e3;
        let cn0_true = 45.0;
        let code = l1c_code_resampled(period as f64, 1, 2 * period);
