...
pub fn acquire_galileo_single(
    ...
    // possible sign flip between the two integrated periods (25-chip
    // secondary code): `[c, c]` and `[c, -c]`.
                ...
                let cn0s: Vec<Option<f64>> = strengths
                    ...
                    .map(|(&v_m, &v_s)| {
                        if v_s > 0.0 && v_m > v_s {
                            ...
                            crate::acr::estimate_cn0(r_a, crate::acr::GALILEO_E1C_ACR_TABLE)
                        } else {
                            None
                        }
                    })
...
mod tests {
    ...
    fn test_cn0_acr_end_to_end() {
        ...
        for (flip, n_trials, lo, hi) in [(false, 60usize, -1.0, 1.0), (true, 120usize, -4.0, -0.5)] {
