...
///
/// For each PRN, the best result (highest signal strength) across the
/// selected antennas is retained.
    ...
    let mut results: Vec<(usize, f64, f64, f64)> = (1..=GALILEO_E1_NUM_SATS)
        ...
        .map(|prn| {
            ...
            for (i, raw) in ant_data.iter().enumerate() {
                ...
                if result.signal_strength > best_strength {
                    best_strength = result.signal_strength;
                    best_phase = result.codephase_frac;
